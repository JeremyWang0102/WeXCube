...
#define TX_BUFFER_LENGTH    512U
#define RX_BUFFER_LENGTH    512U

//The tx buffer and rx buffer for app_drv_fifo
//length should be a power of 2
static uint8_t app_tx_buffer[TX_BUFFER_LENGTH] = {0};
static uint8_t app_rx_buffer[RX_BUFFER_LENGTH] = {0};

static app_drv_fifo_t app_tx_fifo;
app_drv_fifo_t app_rx_fifo;


//fifo length less that MTU-3, retry times
uint32_t app_to_ble_send_evt_cnt = 0;

void app_drv_process(void)
{
    UINT32 irq_status;
    SYS_DisableAllIrq(&irq_status);
    if (wexSendFlag)
    {
        tmos_start_task(Peripheral_TaskID, APP_TO_BLE_SEND_EVT, 2);
        wexSendFlag = false;
    }
...
void app_drv_init()
{
    //tx fifo and tx fifo
    //The buffer length should be a power of 2
    app_drv_fifo_init(&app_tx_fifo, app_tx_buffer, TX_BUFFER_LENGTH);
    app_drv_fifo_init(&app_rx_fifo, app_rx_buffer, RX_BUFFER_LENGTH);
}
...
void on_bleuartServiceEvt(uint16_t connection_handle, ble_uart_evt_t *p_evt)
{
    switch(p_evt->type)
    {
        ...
        case BLE_UART_EVT_BLE_DATA_RECIEVED:
            ...
            //ble to wexcube
            wex_push((uint8_t *)p_evt->data.p_data, p_evt->data.length);
...
// GAP GATT Attributes
static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "USB_HID_BLE";
    ...
    if(events & APP_TO_BLE_SEND_EVT)
    {
        ...
        switch(send_to_ble_state)
        {
            case SEND_TO_BLE_TO_SEND:
                ...
                if(app_drv_fifo_length(&app_rx_fifo) >= read_length)
                {
                    PRINT("FIFO_LEN:%d\r\n", app_drv_fifo_length(&app_rx_fifo));
                    result = app_drv_fifo_read(&app_rx_fifo, to_test_buffer, &read_length);
                    app_to_ble_send_evt_cnt = 0;
                }
                else
                {
                    if(app_to_ble_send_evt_cnt > 10)
                    {
                        result = app_drv_fifo_read(&app_rx_fifo, to_test_buffer, &read_length);
                        app_to_ble_send_evt_cnt = 0;
                    }
                    ...
                    {
                        tmos_start_task(Peripheral_TaskID, APP_TO_BLE_SEND_EVT, 4);
                        app_to_ble_send_evt_cnt++;
                        //PRINT("NO TIME OUT\r\n");
                    }
                }
        ...
        return (events ^ APP_TO_BLE_SEND_EVT);
    }
